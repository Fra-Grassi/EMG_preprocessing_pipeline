...
Stage 0 adds automatically generated provenance fields to the existing `sets` structure immediately before it is saved. These fields record the settings-file timestamp, operating system, MATLAB version, EEGLAB version, relevant plugin versions when available, and the pipeline Git commit when available. Unavailable optional version or Git information is stored as unavailable rather than stopping the pipeline.

The enriched `sets` structure is saved through the existing `preprocessing_settings.mat` and `preprocessing_settings.txt` outputs. No run ID, separate manifest, run-specific directory, cumulative provenance log, or additional CSV column is created.
