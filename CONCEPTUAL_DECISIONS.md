...
Each complete Stage 0 execution generates a filesystem-safe `sets.run_ID` for that settings snapshot and processing batch. Stage 0 continues to save the active `preprocessing_settings.mat` and readable settings text used by later stages. It also initializes a MAT and text run manifest under `resources/run_manifests/`, identified by the run ID. The exact settings snapshot stored in that manifest is immutable; Stages 1 and 2 may append execution, input, and output records without replacing the snapshot or earlier records. Current-settings behavior remains governed by CD-16; no fallback or silent migration is added for older development settings.
