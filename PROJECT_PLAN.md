...
Scientific scope: automatic-rejection thresholds are chosen by each user or researcher for each dataset, based on data quality and research questions. Threshold selection is not a pipeline implementation task.
Agent 6 commits `f3116f5` and `5f16bbb` were integrated as `c2d4bdc` and `7a08a45`. The researcher reported the revised MATLAB tests and a real-data check of the early rejection checkpoint passing on 2026-09-18.
...
CD-14 and CD-15 additionally approve a baseline-to-peak excursion divisor (default 4), user-configured crossing duration in milliseconds, nearest-zero anchor with positive-side tie breaking, a participant-level median mode, and median fallback for missing crossings or omitted epochs with warnings. The threshold is the processed baseline mean plus one divisor fraction of the positive excursion from that baseline to the post-trigger maximum. No positive excursion makes the estimate unavailable. No valid detections, missing photodiode channels, and corrected events outside recording bounds cause errors. See CONCEPTUAL_DECISIONS.md for the complete contract.
...
5. Tier 1.5: rejection accounting — completed and validated with revised MATLAB tests and real data.
