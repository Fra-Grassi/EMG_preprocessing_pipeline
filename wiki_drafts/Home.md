...
- **Decisions:** [CD-01 through CD-18](../CONCEPTUAL_DECISIONS.md#decision-register) are represented as implemented or integrated behavior, subject to the validation limits stated on the individual pages.
...
- **Tests and validation:** [deterministic MAV tests](../tests/run_mav_tests.m), [event-marker normalization tests](../tests/test_event_marker_normalization.m), [file-selection and identity tests](../tests/test_file_selection_identity.m), [rejection-accounting tests](../tests/test_rejection_accounting.m), [production trigger-shifting tests](../tests/test_shift_triggers_production.m), and the recorded [validation baseline](../PROJECT_PLAN.md#validation-baseline).
- **Current status:** Tier 1 implementation tasks are marked complete in `PROJECT_PLAN.md`; the researcher-reported checks and their limits are recorded there. CD-10 validates settings at each stage entry. Automatic-rejection thresholds remain choices for the researcher to make for each dataset, not a pending pipeline-wide implementation decision.
