...
Muscle names and their column order follow `sets.emg_channel_names`, matched to the selected source channels or bipolar pairs as explained in [EMG Channel Selection and Re-referencing](EMG-Channel-Selection-and-Re-referencing.md#output-behavior-and-metadata).
...
- **Tests and recorded validation:** [`validate_mav_output.m`](../tests/validate_mav_output.m) checks keys, paired missingness, and optional rejection-count agreement; [`compare_mav_outputs.m`](../tests/compare_mav_outputs.m) compares keyed output and missingness; [`test_file_selection_identity.m`](../tests/test_file_selection_identity.m) checks text identity; [`test_rejection_accounting.m`](../tests/test_rejection_accounting.m) covers rejection modes, checkpoint timing, absent conditions, participant-key separation, leading-zero IDs, cumulative CSV readback, and missing feature values on restored rejected rows. The researcher reported the extended suite passing in MATLAB R2024b on 2026-09-21. Its added participant-key fixture uses one muscle, raw MAV, and muscle-wise standardization with controlled EEGLAB substitutes; see the [test handoff and limits](../tests/rejection_accounting_handoff.md#tier-2-participant-key-regression-extension-agent-9-2026-09-21). [PROJECT_PLAN.md, validation baseline](../PROJECT_PLAN.md#validation-baseline) records the researcher-run validation.
